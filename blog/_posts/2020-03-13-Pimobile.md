...
The idea behind Imitation Learning is to learn a policy $\pi$ mapping observations to actions : $\pi _{\theta} : \mathcal(O) \leftarrow \mathcal(A)$. The policy would allow to car to know which actions to performs depending on the observation: the camera input, according to the driving we've done before.
